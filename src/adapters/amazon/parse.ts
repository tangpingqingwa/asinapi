...
export function parseSearchHtml(
  ...
  const cardRe =
    /<article\b([^>]*data-hook=["']search-result["'][^>]*)>([\s\S]*?)<\/article>/gi;
  let match: RegExpExecArray | null;
...
function parseSearchCard(openTag: string, inner: string): SearchResult | null {
  ...
  const averageRaw = firstMatch(
    inner,
    /([0-9]+(?:\.[0-9]+)?)\s+out of 5(?:\s+stars)?/i,
  );
  const countRaw =
  ...
  return {
    ...
    image: image !== null && /^https?:\/\//i.test(image) ? image : null,
  };
}
...
export function isUnavailablePage(html: string): boolean {
  return (
...
function extractImages(html: string): string[] {
  ...
  while ((match = imgRe.exec(html)) !== null) {
    const tag = match[0];
    if (
      /id=["']landingImage["']/i.test(tag) ||
      /class=["'][^"']*a-dynamic-image/i.test(tag)
    ) {
...
function parseUsd(display: string): { amount: number | null; display: string | null } {
  ...
  const match = /\$([0-9]{1,3}(?:,[0-9]{3})*|[0-9]+)(?:\.([0-9]{2}))?/.exec(
    trimmed,
  );
...
function extractRating(html: string): Product["rating"] {
  ...
  const countMatch = /([0-9]{1,3}(?:,[0-9]{3})*|[0-9]+)\s+ratings?/i.exec(
    countText,
  );
  return {
    average: averageMatch?.[1] !== undefined ? Number(averageMatch[1]) : null,
    count:
...
function extractAttributes(html: string): Record<string, string> {
  ...
  const rowRe =
    /<tr\b[^>]*>\s*<th\b[^>]*>([\s\S]*?)<\/th>\s*<td\b[^>]*>([\s\S]*?)<\/td>/gi;
  let match: RegExpExecArray | null;
