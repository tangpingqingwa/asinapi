...
export type KeyPrefix = "ak_live" | "ak_test";
...
export function prefixFromSecret(secret: string): KeyPrefix | null {
  if (secret.startsWith("ak_test_")) {
    return "ak_test";
  }
  if (secret.startsWith("ak_live_")) {
    return "ak_live";
  }
  return null;
}
...
export function createKey(
  ...
  if (prefix === null) {
    throw new Error("API key must start with ak_live_ or ak_test_");
  }
  const key: Key = {
...
export function lookupKey(db: AsinApiDb, secret: string): Key | null {
...
export function bootstrapKeyIfEmpty(db: AsinApiDb, secret: string): Key | null {
