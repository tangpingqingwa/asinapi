...
const TEST_KEY = "ak_test_bootstrap_fixture";
...
test("loadConfig requires ASINAPI_DATABASE in production", () => {
  assert.throws(
    () => loadConfig({ NODE_ENV: "production" }),
    /ASINAPI_DATABASE is required in production/,
  );
  const config = loadConfig({
    NODE_ENV: "production",
    ASINAPI_DATABASE: "/tmp/asinapi.sqlite",
    ASINAPI_BOOTSTRAP_KEY: "ak_test_dev",
  });
...
test("createKey stores a hash and lookupKey finds the row", () => {
  ...
  const secret = "ak_live_unit_fixture";
  const created = createKey(db, { secret, credits: 7 });
  const found = lookupKey(db, secret);
  assert.equal(found?.id, created.id);
  assert.equal(found?.prefix, "ak_live");
  assert.equal(found?.credits, 7);
  assert.equal(lookupKey(db, "ak_live_unknown"), null);
  assert.equal(lookupKey(db, "not-a-key"), null);
  assert.equal(lookupKey(db, "ck_live_clipapi_prefix"), null);
  assert.throws(() => createKey(db, { secret: "nope" }), /ak_live_|ak_test_/);
...
test("GET /v1/me without bearer is 401 with 0 credits", async () => {
  ...
  const cases = [
    ...
    { authorization: "Bearer ak_test_unknown" },
    { authorization: "Bearer ck_live_wrong_product" },
  ];
...
test("bootstrap test key → GET /v1/me shows credits", async () => {
  ...
  assert.equal(body.data.key.prefix, "ak_test");
